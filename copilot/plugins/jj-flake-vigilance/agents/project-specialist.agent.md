...
9. Auto-merge is opt-in: only request it for non-draft same-repository PRs that carry the `automerge` label, and rely on `.github/workflows/automerge.yml` plus the repo's required checks to merge safely after CI passes.
